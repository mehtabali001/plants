		...
		<div class="page-title-box">
			<div class="float-right" style="margin-bottom: 15px;">
				<div class="btn-group" role="group" aria-label="Basic outlined example">
                     <? if(!empty($role_permissions) && in_array(243,$role_permissions)) { ?>    
                        <a type="button" id="print_report" onclick="print_ledger('customer_ledger_print');" class="btn btn-outline-primary  print_report" name="" value="" disabled><i class="fa fa-print" aria-hidden="true"></i>&nbsp;Print</a>
					    <? } ?>
					    <? if(!empty($role_permissions) && in_array(244,$role_permissions)) { ?>
                        <a type="button" id="pdf_purchase_report" class="btn  btn-outline-primary pdf_purchase_report" onclick="pdf_ledger('customer_ledger_pdf');" name="" value="" disabled><i class="fa fa-file-pdf-o" aria-hidden="true"></i>&nbsp;PDF</a>
						<? } ?>
...
<table id="datatable_tb" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
	...
	<tbody class="ledgerRows" id="ledgerRows">
					...
					<tr>
						...
						<td >
						    ...
						    if($ledgedet['type']=='JV'){
        										    ...
        										    ?>
        										 <a href="<?php if($url=='#'){echo'javascript:void(0)';}else{echo base_url().'Vouchers/'.$url.'/edit/'.$ledgedet['v_id'];}?>" ><?php echo $ledgedet['type'];?> - <?php echo $ledgedet['v_id'];?></a></td>
		...
		<tr class="search_finalsum totals">
		    <td style="display:none"></td>
		    <td style="display:none"></td>
		    <td style="display:none"></td>
...
<script>
    ...
    function downloadcsv(){
       ...
       var url="<?= base_url();?>/Accounts/customer_ledger_filter_csv?"+formdata;
       window.open(url);
   }
</script>
