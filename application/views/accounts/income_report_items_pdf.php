...
@media print{
    hr{
        width:100%;margin-top:10px;z-index:100000;
    }
}
...
<?php 
    ...
    ?>
    <h2 style="float:left;margin-top: 4%;font-size:16px;">Income statement.'<?=$item['fld_category']?>'</h2>
    <!--<img class="company-logo" src="https://erp.mktechsol.com//assets/custom_elements/images/logo.png" style="margin-top: -12%;height:120px;float:right;">-->
    <? $settings=$this->db->query("SELECT * FROM tbl_general_settings WHERE setting_id = 1")->row_array();?>
                <img class="company-logo" src="<?=base_url()?>/assets/uploads/logo/<?=$settings['system_logo'];?>" style="margin-top: -12%;height:120px;float:right;">
