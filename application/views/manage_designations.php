                                    ...
                                    <table id="datatable_tb" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
										...
										<?php if($designations){
											foreach($designations as $desig){
											    $updateby =	$this->Common_model->select_single_field('fld_username','tbl_users',array('fld_id'=>$desig['fld_updated_by']));
											?>
                                        <tr>
                                            <td><?= ucfirst($desig['designation_name'])."<br>";
                                    		    if($desig['fld_updated_by'] != 0){
                                    				echo '<span style="width: 170px; display: inline-block;color:#506ee4; font-size:10px;">Updated On: '.date("d-m-Y,h:i A",strtotime(@$desig['fld_updated_date'])).'</span>'."<br>";
                                    				echo '<span style="width: 170px; display: inline-block;color:#21d0c0; font-size:10px;">By: '.$updateby.'</span>';
                                    		    }?></td>
                                            <td><?= ucfirst($desig['designation_detail']);?></td>
                                            <td><?php if($desig['deleted'] == 1){
												echo 'Disable';
											}elseif($desig['deleted'] == 0){
												echo 'Enable';
											}else{
												echo '';
											} ?></td>
                                            <td><?= date('d-M-Y',strtotime($desig['created_date']));?></td>
                                            <td>
											<? if(!empty($role_permissions) && in_array(101,$role_permissions)) { ?>
											<a href="<?= base_url('Common/editDesignation/'.$desig['id'].'')?>">
											...
											<!--</a>-->
											<a data-toggle="modal" class="exampleModal" data-target="#exampleModal" data-content="<?php echo $desig['id']; ?>" data-uri="<?= base_url('Common/deleteDesignation/'.$desig['id'].'')?>"><i style="font-size:20px;cursor:pointer;" class="mdi mdi-delete-forever" title="Delete"></i></a>
											<? } ?>
											</td>
                                        </tr>
										<?php }}else{?>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content" style="box-shadow: 0 7px 14px 0 rgb(65 203 216 / 50%) !important;">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="exampleModalLabel">Delete Designation</h5>
