		...
		<div class="page-title-box">
			<div class="float-right">
				<div class="btn-group" role="group" aria-label="Basic outlined example">
                  ...
                  <a href="<?= base_url();?>Common/manage_Unit" type="button" class="btn btn-outline-primary"><i class="fa fa-eye"></i>&nbsp;view Unit</a>
                </div>
			</div>
                                    ...
                                    <table id="datatable_tb" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
										...
										<?php if($banks){
											foreach($banks as $bank){
											    $updateby =	$this->Common_model->select_single_field('fld_username','tbl_users',array('fld_id'=>$bank['fld_updated_by']));
											?>
                                        <tr>
                                            <td><?= ucfirst($bank['fld_bank'])."<br>";
                                    		    if($bank['fld_updated_by'] != 0){
                                    				echo '<span style="width: 170px; display: inline-block;color:#506ee4; font-size:10px;">Updated On: '.date("d-m-Y,h:i A",strtotime(@$bank['fld_updated_date'])).'</span>'."<br>";
                                    				echo '<span style="width: 170px; display: inline-block;color:#21d0c0; font-size:10px;">By: '.$updateby.'</span>';
                                    		    }?></td>
                                            <td><?= ucfirst($bank['fld_account_title']);?></td>
                                            <td><?= ucfirst($bank['fld_accountnumber']);?></td>
                                            <td><?= ucfirst($bank['fld_address']);?></td>
                                            <td><?php if($bank['fld_status'] == 1){
												echo 'Enable';
											}elseif($bank['fld_status'] == 0){
												echo 'Disable';
											}else{
												echo '';
											} ?></td>
                                            <td><?= date('d-M-Y',strtotime($bank['fld_created_date']));?></td>
                                            <td>
											<? if(!empty($role_permissions) && in_array(89,$role_permissions)) { ?>
											<a href="<?= base_url('Common/editBank/'.$bank['fld_id'].'')?>">
											...
											<!--</a>-->
											<a data-toggle="modal" class="exampleModal" data-target="#exampleModal" data-content="<?php echo $bank['fld_id']; ?>" data-uri="<?= base_url('Common/deleteBank/'.$bank['fld_id'].'')?>"><i style="font-size:20px;cursor:pointer;" class="mdi mdi-delete-forever" title="Delete"></i></a>
											<? } ?>
											</td>
                                        </tr>
										<?php }}else {?>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content" style="box-shadow: 0 7px 14px 0 rgb(65 203 216 / 50%) !important;">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="exampleModalLabel">Delete Bank</h5>
