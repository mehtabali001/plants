		...
		<div class="page-title-box">
			<div class="float-right">
			    <div class="btn-group" role="group" aria-label="Basic outlined example">
                  <a href="<?= base_url();?>Common/addDepartment" type="button" class="btn btn-outline-primary"><i class="fa fa-shopping-cart"></i>&nbsp;New Department</a>
                  <a href="<?= base_url();?>Common/manage_departments" type="button" class="btn btn-primary btn-large"><i class="fa fa-eye"></i>&nbsp;View Departments</a>
                                    ...
                                    <table id="datatable_tb" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
										...
										<?php if($departments){
											foreach($departments as $depart){
											     $updateby =	$this->Common_model->select_single_field('fld_username','tbl_users',array('fld_id'=>$depart['fld_updated_by']));
											?>
                                        <tr>
                                            <td><?= ucfirst($depart['department_name'])."<br>";
                                    		    if($depart['fld_updated_by'] != 0){
                                    				echo '<span style="width: 170px; display: inline-block;color:#506ee4; font-size:10px;">Updated On: '.date("d-m-Y,h:i A",strtotime(@$depart['fld_updated_date'])).'</span>'."<br>";
                                    				echo '<span style="width: 170px; display: inline-block;color:#21d0c0; font-size:10px;">By: '.$updateby.'</span>';
                                    		    }?></td>
                                            <td><?= ucfirst($depart['department_detail']);?></td>
                                            <td><?php if($depart['deleted'] == 1){
												echo 'Disable';
											}elseif($depart['deleted'] == 0){
												echo 'Enable';
											}else{
												echo '';
											} ?></td>
                                            <td><?= date('d-M-Y',strtotime($depart['created_date']));?></td>
                                            <td>
											<? if(!empty($role_permissions) && in_array(97,$role_permissions)) { ?>
											<a href="<?= base_url('Common/editDepartment/'.$depart['id'].'')?>">
											...
											<!--</a>-->
											<a data-toggle="modal" class="exampleModal" data-target="#exampleModal" data-content="<?php echo $depart['id']; ?>" data-uri="<?= base_url('Common/deleteDepartment/'.$depart['id'].'')?>"><i style="font-size:20px;cursor:pointer;" class="mdi mdi-delete-forever" title="Delete"></i></a>
											<? } ?>
											</td>
                                        </tr>
										<?php }}else{?>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content" style="box-shadow: 0 7px 14px 0 rgb(65 203 216 / 50%) !important;">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="exampleModalLabel">Delete Department</h5>
