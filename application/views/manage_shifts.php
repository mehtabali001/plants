		...
		<div class="page-title-box">
			<div class="float-right">
				<div class="btn-group" role="group" aria-label="Basic outlined example">
                  <a href="<?= base_url();?>Common/addShift" type="button" class="btn btn-outline-primary"><i class="fa fa-shopping-cart"></i>&nbsp;New Shift</a>
                  <a href="<?= base_url();?>Common/manage_shifts" type="button" class="btn btn-primary btn-large"><i class="fa fa-eye"></i>&nbsp;View Shifts</a>
                                ...
                                <div class="card-body">
                                    ...
                                    <table id="datatable_tb" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
										...
										<?php if($shifts){
											...
											?>
                                        <tr>
                                            <td><?= ucfirst($shif['shift_name'])."<br>";
                                    		    if($shif['fld_updated_by'] != 0){
                                    				echo '<span style="width: 170px; display: inline-block;color:#506ee4; font-size:10px;">Updated On: '.date("d-m-Y,h:i A",strtotime(@$shif['fld_updated_date'])).'</span>'."<br>";
                                    				echo '<span style="width: 170px; display: inline-block;color:#21d0c0; font-size:10px;">By: '.$updateby.'</span>';
                                    		    }?></td>
                                            <td><?= ucfirst($shif['shift_detail']);?></td>
                                            <td><?php if($shif['deleted'] == 1){
												echo 'Disable';
											}elseif($shif['deleted'] == 0){
												echo 'Enable';
											}else{
												echo '';
											} ?></td>
                                            <td><?= date('d-M-Y',strtotime($shif['created_date']));?></td>
                                            <td>
											<? if(!empty($role_permissions) && in_array(105,$role_permissions)) { ?>
											<a href="<?= base_url('Common/editShift/'.$shif['id'].'')?>">
											...
											<!--</a>-->
											<a data-toggle="modal" class="exampleModal" data-target="#exampleModal" data-content="<?php echo $shif['id']; ?>" data-uri="<?= base_url('Common/deleteShift/'.$shif['id'].'')?>"><i style="font-size:20px;cursor:pointer;" class="mdi mdi-delete-forever" title="Delete"></i></a>
											<? } ?>
											</td>
                                        </tr>
										<?php }}else{?>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content" style="box-shadow: 0 7px 14px 0 rgb(65 203 216 / 50%) !important;">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="exampleModalLabel">Delete Shift</h5>
