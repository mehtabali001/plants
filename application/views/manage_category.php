                                    ...
                                    <table id="datatable_tb" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
										...
										<?php if($categories){
											foreach($categories as $cate){
											     $updateby =	$this->Common_model->select_single_field('fld_username','tbl_users',array('fld_id'=>$cate['fld_updated_by']));
											?>
                                        <tr>
                                            <td><?= ucfirst($cate['fld_category'])."<br>";
                                    		    if($cate['fld_updated_by'] != 0){
                                    				echo '<span style="width: 170px; display: inline-block;color:#506ee4; font-size:10px;">Updated On: '.date("d-m-Y,h:i A",strtotime(@$cate['fld_updated_date'])).'</span>'."<br>";
                                    				echo '<span style="width: 170px; display: inline-block;color:#21d0c0; font-size:10px;">By: '.$updateby.'</span>';
                                    		    }?></td>
                                            <td><?php if($cate['fld_status'] == 1){
												echo 'Enable';
											}elseif($cate['fld_status'] == 0){
												echo 'Disable';
											}else{
												echo '';
											} ?></td>
                                            <td><?= date('d-M-Y',strtotime($cate['fld_created_date']));?></td>
                                            <td>
											<? if(!empty($role_permissions) && in_array(69,$role_permissions)) { ?>
											<a href="<?= base_url('Common/edit/'.$cate['fld_id'].'')?>">
											...
											<!--</a>-->
											<a data-toggle="modal" class="exampleModal" data-target="#exampleModal" data-content="<?php echo $cate['fld_id']; ?>" data-uri="<?= base_url('Common/delete/'.$cate['fld_id'].'')?>"><i style="font-size:20px;cursor:pointer;" class="mdi mdi-delete-forever" title="Delete"></i></a>
											<? } ?>
											</td>
                                        </tr>
										<?php }}else{?>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content" style="box-shadow: 0 7px 14px 0 rgb(65 203 216 / 50%) !important;">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="exampleModalLabel">Delete Category</h5>
