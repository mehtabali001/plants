		...
		<div class="page-title-box">
			<div class="float-right">
				<div class="btn-group" role="group" aria-label="Basic outlined example">
                  ...
                  <a href="<?= base_url();?>Common/manage_Unit" type="button" class="btn btn-primary btn-large"><i class="fa fa-eye"></i>&nbsp;View Unit</a>
                                    ...
                                    <table id="datatable_tb" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
										...
										<?php if($units){
											foreach($units as $unit){
											    $updateby =	$this->Common_model->select_single_field('fld_username','tbl_users',array('fld_id'=>$unit['fld_updated_by']));
											?>
                                        <tr>
                                            <td><?= ucfirst($unit['fld_unit'])."<br>";
                                    		    if($unit['fld_updated_by'] != 0){
                                    				echo '<span style="width: 170px; display: inline-block;color:#506ee4; font-size:10px;">Updated On: '.date("d-m-Y,h:i A",strtotime(@$unit['fld_updated_date'])).'</span>'."<br>";
                                    				echo '<span style="width: 170px; display: inline-block;color:#21d0c0; font-size:10px;">By: '.$updateby.'</span>';
                                    		    }?></td>
                                            <td><?php if($unit['fld_status'] == 1){
												echo 'Enable';
											}elseif($unit['fld_status'] == 0){
												echo 'Disable';
											}else{
												echo '';
											} ?></td>
                                            <td><?= date('d-M-Y',strtotime($unit['fld_created_date']));?></td>
                                            <td>
											<? if(!empty($role_permissions) && in_array(81,$role_permissions)) { ?>
											<a href="<?= base_url('Common/editUnit/'.$unit['fld_id'].'')?>">
											...
											<!--</a>-->
											<a data-toggle="modal" class="exampleModal" data-target="#exampleModal" data-content="<?php echo $unit['fld_id']; ?>" data-uri="<?= base_url('Common/deleteUnit/'.$unit['fld_id'].'')?>"><i style="font-size:20px;cursor:pointer;" class="mdi mdi-delete-forever" title="Delete"></i></a>
											<? } ?>
											</td>
                                        </tr>
										<?php }} else {?>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content" style="box-shadow: 0 7px 14px 0 rgb(65 203 216 / 50%) !important;">
            <div class="modal-header">
                <h5 class="modal-title mt-0" id="exampleModalLabel">Delete Unit</h5>
