...
class Users extends CI_Model {
    ...
    function check_valid_user($email, $password) {
        $password = md5("kotal" . $password);
        $this->db->where(array('fld_email' => $email, 'fld_password' => $password, 'fld_status' => 1));
    ...
    public function user_activity_log() {
		...
		$group_by='date(tbl_activity_log.fld_added_date)';
		$select="date(tbl_activity_log.fld_added_date) as filter_text,date(tbl_activity_log.fld_added_date) as filter_value";
		switch ($filter) {
		  case "Date_Wise":
			$group_by = "date(tbl_activity_log.fld_added_date)";
			$select="date(tbl_activity_log.fld_added_date) as filter_text,date(tbl_activity_log.fld_added_date) as filter_value";
			break;
		  case "User_Wise":
			$group_by = "tbl_activity_log.fld_user_id";
			$select="tbl_users.fld_username as filter_text,tbl_activity_log.fld_user_id as filter_value";
			break;
		case "Action_Wise":
			$group_by = "tbl_activity_log.fld_action";
			$select="tbl_activity_log.fld_action as filter_text,tbl_activity_log.fld_action as filter_value";
			break;
		case "Login_Wise":
			$group_by = "date(tbl_activity_log.fld_added_date)";
			$select="date(tbl_activity_log.fld_added_date) as filter_text,date(tbl_activity_log.fld_added_date) as filter_value";
			$where="fld_action='LOGIN'";
			break;
		case "Logout_Wise":
			$group_by = "date(tbl_activity_log.fld_added_date)";
			$select="date(tbl_activity_log.fld_added_date) as filter_text,date(tbl_activity_log.fld_added_date) as filter_value";
			$where="fld_action='LOGOUT'";
			break;
		
		}
		$start=str_replace('/', '-', $this->input->post('from_date'));
		$end=str_replace('/', '-', $this->input->post('to_date'));
		$date="date(tbl_activity_log.fld_added_date) between '".date("Y-m-d",strtotime($start))."' AND '".date("Y-m-d",strtotime($end))."'";
		
        $this->db->select(''.$select.'');
        $this->db->from('tbl_activity_log');
        $this->db->join('tbl_users', 'tbl_users.fld_id = tbl_activity_log.fld_user_id');
		...
		if($result){
			foreach($result as $key => $res){
				$logdet=$this->getLogDetail($res['filter_value'],$group_by);
				
				$result[$key]['detail']=$logdet;
			}
		}
	...
	public function getLogDetail($id,$group_by){
	    
	    $where="";
		$filter=$this->input->post('filter');
	    switch ($filter) {
		case "Login_Wise":
			$where="fld_action='LOGIN' OR fld_action='LOGOUT'";
			break;
		case "Logout_Wise":
			$where="fld_action='LOGOUT'";
			break;
		...
		$end=str_replace('/', '-', $this->input->post('to_date'));
		$date="date(tbl_activity_log.fld_added_date) between '".date("Y-m-d",strtotime($start))."' AND '".date("Y-m-d",strtotime($end))."'";
        $this->db->select('tbl_activity_log.*,tbl_users.fld_username');
        $this->db->from('tbl_activity_log');
        $this->db->join('tbl_users', 'tbl_users.fld_id = tbl_activity_log.fld_user_id');
		if($start != "" && $end != ""){
		$this->db->where($date);
    ...
    public function user_activity_log_filter() {
		
		$start=str_replace('/', '-', $this->input->post('from_date'));
		$end=str_replace('/', '-', $this->input->post('to_date'));
		$date="date(al.fld_added_date) between '".date("Y-m-d",strtotime($start))."' AND '".date("Y-m-d",strtotime($end))."'";
        $this->db->select('al.*,u.fld_username');
        $this->db->from('tbl_activity_log al');
        $this->db->join('tbl_users u', 'u.fld_id = al.fld_user_id');
		$this->db->where($date);
        $query = $this->db->get();
    ...
    public function user_activity_log_get() {
		$conditions="";
		$where="";
