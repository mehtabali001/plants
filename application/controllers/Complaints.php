...
class Complaints extends My_controller {
    ...
    public function new_complaint()
    {
		...
		$data['users']=$this->db->query("SELECT * FROM tbl_users where fld_id NOT IN ('".$_SESSION['user_id']."')")->result_array();
		...
		$getMaxID=$this->Complaint_model->getMaxCompID();
		$data['maxid']= 'CI-'.sprintf('%04d', $getMaxID['Auto_increment']);
	...
	public function filter(){
		...
		$data['my_complaints']=$this->Complaint_model->filter();
		$html=$this->load->view('complaints/my_complain_filter',$data,true);
		echo json_encode(array("html"=>$html));
	}
	public function assignfilter(){
		
		$data['my_complaints']=$this->Complaint_model->assignfilter();
		$html=$this->load->view('complaints/assign_complain_filter',$data,true);
		echo json_encode(array("html"=>$html));
	}
